...
let rec main env = function
  ...
  | Syntax.Not(e) -> main env (Syntax.If(e, Syntax.Bool(false), Syntax.Bool(true)))
  | Syntax.Neg(e) ->
      insert_let (main env e)
        (fun x -> Neg(x), Type.Int)
  | Syntax.Add(e1, e2) ->
      insert_let (main env e1)
        (fun x -> insert_let (main env e2)
  ...
  | Syntax.Eq _ | Syntax.LE _ as cmp ->
      main env (Syntax.If(cmp, Syntax.Bool(true), Syntax.Bool(false)))
  | Sytax.If(Syntax.Not(e1), e2, e3) -> main env (Syntax.If(e1, e3, e2))
  | Syntax.If(Syntax.Eq(e1, e2), e3, e4) ->
      insert_let (main env e1)
        (fun x -> insert_let (main env e2)
            (fun y ->
               let e3', t3 = main env e3 in
               let e4', t4 = main env e4 in
               IfEq(x, y, e3', e4'), t3))
      ...
      insert_let (main env e1)
        (fun x -> insert_let (main env e2)
            (fun y ->
               ...
               let e4', t4 = main env e4 in
               IfLE(x, y, e3', e4'), t3))
  | Syntax.If(e1, e2, e3) -> main env (Syntax.If(Syntax.Eq(e1, Syntax.Bool(false)), e3, e2))
  | Syntax.Let((x, t), e1, e2) ->
      let e1', t1 = main env e1 in
      let e2', t2 = main (MiniMap.add x t env) e2 in
      Let((x, t), e1', e2), t2
