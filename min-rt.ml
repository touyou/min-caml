...
let rec solver_rect_surface m dirvec b0 b1 b2 i0 i1 i2  =
  if fiszero dirvec.(i0) then false else
    ...
    let d = fneg_cond ((o_isinvert m) lxor (fisneg dirvec.(i0))) abc.(i0) in
...
let rec setup_rect_table vec m =
  ...
  if fiszero vec.(0) then (* YZ平面 *)
    ...
    (* 面の X 座標 *)
    const.(0) <- fneg_cond ((o_isinvert m) lxor (fisneg vec.(0))) (o_param_a m);
  ...
  if fiszero vec.(1) then (* ZX平面 : YZ平面と同様*)
    ...
    const.(2) <- fneg_cond ((o_isinvert m) lxor (fisneg vec.(1))) (o_param_b m);
    const.(3) <- 1.0 /. vec.(1)
  );
  if fiszero vec.(2) then (* XY平面 : YZ平面と同様*)
    const.(5) <- 0.0
  else (
    const.(4) <- fneg_cond ((o_isinvert m) lxor (fisneg vec.(2))) (o_param_c m);
    const.(5) <- 1.0 /. vec.(2)
  );
  const
in
...
let rec is_plane_outside m p0 p1 p2 =
  ...
  not ((o_isinvert m) lxor (fisneg w))
in
...
let rec is_second_outside m p0 p1 p2 =
  ...
  not ((o_isinvert m) lxor (fisneg w2))
in
...
let rec shadow_check_one_or_matrix ofs or_matrix =
  ...
  if (* range primitive が無いか、またはrange_primitiveと交��る事を確認 *)
